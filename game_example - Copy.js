...
var mspeed = new Float32Array(999);
...
function alter_ocean_1_cb(in_params, out_params){//NO STORM
out_params[0]=.2;
out_params[1]=-.2;
}

function alter_ocean_2_cb(in_params, out_params){//STORM
out_params[0]=2;
out_params[1]=-2;
}

function test1(){
//m_logic.run_entrypoint("Scene", "B4WLogicNode");

m_logic_n.append_custom_callback("alter_ocean_node", alter_ocean_1_cb);
...
m_trans.set_translation(sphere,-13.4064,1.99513,19.1242);
...
//testvar6 = m_time.get_timeline();
 ///////////////////////////////////TESTING//////////////////////
 m_phy.set_character_move_type(_character, m_phy.CM_WALK);
//  testvar3 = testvar3 + 1;
}
...
function update_vehicle(){
 if (playerboat != null){
		var boatspeed=5;
 m_trans.move_local(playerboat, 0,boatdir * elapsedtime * boatspeed, 0);
 }
}
...
var ocean = m_scs.get_object_by_name("ocean");
 ...
 m_trans.move_local(ocean, 0,2 * elapsedtime * -1,0);
 m_trans.move_local(ocean,2 * elapsedtime * -1,0,0);


char_pos= m_trans.get_translation(_character);///////SCROLL OCEAN VIA KNOCKBACK
ocean_pos= m_trans.get_translation(ocean);
var tempdistx = char_pos[0]-ocean_pos[0];
var tempdisty= char_pos[1]-ocean_pos[1];
if (tempdisty < -100){
 m_trans.move_local(ocean, 0,-100 , 0);
}
if ( tempdisty > 100){
 m_trans.move_local(ocean, 0,100 , 0);
}
if (tempdistx < -100){
 m_trans.move_local(ocean, -100,0 , 0);
}
if ( tempdistx > 100){
 m_trans.move_local(ocean, 100,0 , 0);
}
...
obj=_character;
get_translation(obj, player_pos);
m_vec3.copy(player_pos,target_pos);//works !
m_vec3.copy(player_pos,vec3_tmp);
player_pos[2]=player_pos[2]+1;
target_pos[2]=target_pos[2]-1;

tracingobject = obj;
if (character_ray_test == null){
character_ray_test =	m_phy.append_ray_test_ext(null, player_pos,target_pos ,"ANY", gravitycallback, false, false, true, false);
}


/*
tracesuccess = playerhitfract;
testvar1=playerhitfract;
testvar2=phit_pos[2];
if (playerhitfract >.093){
	 	m_trans.move_local(_character, 0, 0, -.01);//fall
}
if (playerhitfract <.091){
	set_translation(_character,player_pos[0],player_pos[1],phit_pos[2]);
}
*/
...
get_translation(_character, player_pos);
m_trans.set_translation(_character,player_pos[0],player_pos[1],hit_pos[2]);

//playerhitfract=hit_fract;
//m_vec3.copy(hit_pos,phit_pos);
...
//var x=player_pos[0];
//var y=player_pos[1];
set_translation(dummy,player_pos[0],player_pos[1],rayhitpos[2]);
...
if (m_trans.distance(tracingobject, dummy) < 2){
 if (playerjumpenergy <= 0){
	 playerjumpenergy = 0;
 //matchposgravity(dummy,tracingobject);//test??
	} else {
...
if (obj_hit != null) {
//move(obj_hit,.1);//test
tracedobject = obj_hit;//this is where I can tell what the character is standing on
//testvar2 = m_scs.get_object_name(obj_hit);
}
